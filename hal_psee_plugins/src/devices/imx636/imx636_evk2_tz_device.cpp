...
TzEvk2Imx636::TzEvk2Imx636(std::shared_ptr<BoardCommand> cmd, uint32_t dev_id,
...
std::shared_ptr<TzDevice> TzEvk2Imx636::build(std::shared_ptr<BoardCommand> cmd, uint32_t dev_id,
                                              std::shared_ptr<TzDevice> parent) {
...
bool TzEvk2Imx636::can_build(std::shared_ptr<BoardCommand> cmd, uint32_t dev_id) {
...
TzEvk2Imx636::~TzEvk2Imx636() {}

void TzEvk2Imx636::start() {
    TzIssdDevice::start();
}

void TzEvk2Imx636::stop() {
    TzIssdDevice::stop();
}

long long TzEvk2Imx636::get_sensor_id() {
    return (*register_map)[SENSOR_PREFIX + "chip_id"].read_value();
}

std::list<StreamFormat> TzEvk2Imx636::get_supported_formats() const {
    std::list<StreamFormat> formats;
    formats.push_back(StreamFormat("EVT3;height=720;width=1280"));
    return formats;
}

StreamFormat TzEvk2Imx636::get_output_format() const {
    return StreamFormat("EVT3;height=720;width=1280");
}

long TzEvk2Imx636::get_system_id() {
    return cmd->read_device_register(tzID, 0x800)[0];
}

bool TzEvk2Imx636::set_mode_standalone() {
...
bool TzEvk2Imx636::set_mode_master() {
    /* time_base_config(ext_sync, master, master_sel, fwd_up, fwd_down)
        - ext_sync   = 1 --|
        - master     = 1 ----> generate internal sync_out
        - master_sel = 0 -> Don't care since master = 1. Time-base counter is auto-generated.
...
bool TzEvk2Imx636::set_mode_slave() {
    /* time_base_config(ext_sync, master, master_sel, fwd_up, fwd_down)
        - ext_sync   = 1 --|
...
I_CameraSynchronization::SyncMode TzEvk2Imx636::get_mode() const {
    return sync_mode_;
}

int TzEvk2Imx636::get_temperature() {
...
int TzEvk2Imx636::get_illumination() {
    bool valid       = false;
    uint16_t retries = 0;
    uint32_t counter = 0;
...
void TzEvk2Imx636::temperature_init() {
...
void TzEvk2Imx636::lifo_control(bool enable, bool out_en, bool cnt_en) {
...
void TzEvk2Imx636::iph_mirror_control(bool enable) {
