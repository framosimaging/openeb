...
#include "devices/imx636/imx636_rdk2_tz_device.h"
...
TzRdk2Imx636::TzRdk2Imx636(std::shared_ptr<BoardCommand> cmd, uint32_t dev_id,
    ...
    TzDeviceWithRegmap(Imx636Evk2RegisterMap, Imx636Evk2RegisterMapSize, ROOT_PREFIX),
...
bool TzRdk2Imx636::can_build(std::shared_ptr<BoardCommand> cmd, uint32_t dev_id) {
    ...
    if (is_enable.get32(1) == 0) {
        TzGenericCtrlFrame enable(TZ_PROP_DEVICE_ENABLE | TZ_WRITE_FLAG);
        enable.push_back32(dev_id);
        enable.push_back32(1);
        cmd->transfer_tz_frame(enable);
    }

    return (cmd->read_device_register(dev_id, 0x800)[0] == SYSTEM_RDK2_IMX636);
}
static TzRegisterBuildMethod method("psee,video_rdk2_imx636", TzRdk2Imx636::build, TzRdk2Imx636::can_build);

void TzRdk2Imx636::spawn_facilities(DeviceBuilder &device_builder, const DeviceConfig &device_config) {
...
I_HW_Identification::SensorInfo TzRdk2Imx636::get_sensor_info() {
    return {4, 2, "IMX636"};
}

std::list<StreamFormat> TzRdk2Imx636::get_supported_formats() const {
    std::list<StreamFormat> formats;
    formats.push_back(StreamFormat("EVT3;height=720;width=1280"));
    return formats;
}

StreamFormat TzRdk2Imx636::get_output_format() const {
    return StreamFormat("EVT3;height=720;width=1280");
}

long TzRdk2Imx636::get_system_id() {
    return cmd->read_device_register(tzID, 0x800)[0];
}

bool TzRdk2Imx636::set_mode_standalone() {
...
bool TzRdk2Imx636::set_mode_master() {
    /* time_base_config(ext_sync, master, master_sel, fwd_up, fwd_down)
        - ext_sync   = 1 --|
        - master     = 1 ----> generate internal sync_out
        - master_sel = 0 -> Don't care since master = 1. Time-base counter is auto-generated.
...
bool TzRdk2Imx636::set_mode_slave() {
    /* time_base_config(ext_sync, master, master_sel, fwd_up, fwd_down)
        - ext_sync   = 1 --|
...
I_CameraSynchronization::SyncMode TzRdk2Imx636::get_mode() {
    return sync_mode_;
}

int TzRdk2Imx636::get_temperature() {
...
int TzRdk2Imx636::get_illumination() {
    bool valid       = false;
    uint16_t retries = 0;
    uint32_t counter = 0;
...
void TzRdk2Imx636::temperature_init() {
...
void TzRdk2Imx636::lifo_control(bool enable, bool out_en, bool cnt_en) {
...
void TzRdk2Imx636::iph_mirror_control(bool enable) {
